        ...
        <div className={playerClassName} style={playerStyle} onMouseDown={handleSwipeStart} onTouchStart={handleSwipeStart}>
            ...
            <div className="peek-modal-peek-header">
                ...
                <div className="peek-peek-controls">
                    ...
                    <button type="button" onClick={onRemoveBeat} className="text-gray-400 hover:text-white p-1"><TrashIcon/></button>
                </div>
            </div>
            <div className="peek-modal-expanded-content">
                <div className="flex justify-between items-center mb-2">
                    <div className="text-sm">
                        <p className="text-gray-200 font-bold">Master Beat</p>
                        <p className="text-gray-400 truncate max-w-xs">{beat.file.name}</p>
                    </div>
                     <button type="button" onClick={onRemoveBeat} className="text-gray-400 hover:text-white p-1"><TrashIcon/></button>
                </div>
                <div ref={waveformContainerRef} className="relative h-16 w-full flex items-center cursor-pointer my-2" onMouseDown={handleScrubStart} onTouchStart={handleScrubStart}>
                    {isLoadingWaveform ? (
                         <div className="w-full h-full flex items-center justify-center text-xs text-gray-500">Loading beat...</div>
                    ) : (
